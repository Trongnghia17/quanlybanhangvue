          ...
          <v-list-item @click="addProductToOrder(item)">
            ...
            <v-list-item-content>
              <v-list-item-title>
                ...
                <v-spacer></v-spacer>
                {{ formatCurrency(item.base_cost) }} - Tồn kho: {{ item.temporality }}
              </v-list-item-title>
            </v-list-item-content>
          </v-list-item>
        </template>
    ...
    <v-row no-gutters class="flex-grow-1">
      ...
      <v-col cols="12" md="4" class="order-summary pa-4">
        <v-autocomplete
          ...
          :items="listSupplier"
          placeholder="Chọn nhà cung cấp (F4)"
...
export default {
  ...
  data() {
    return {
      ...
      orderDiscountType: 2, // Default to fixed amount (VND)
      searchQuery: '',
      isSearching: false,
  ...
  computed: {
    ...
    totalAmount() {
      return this.selectedProducts.reduce((sum, product) => {
        return sum + (product.base_cost * product.quantity);
      }, 0);
    },
    ...
    // Calculate the final amount that needs to be paid (products after discount minus order discount)
    amountDue() {
    ...
    changeLabel() {
      return this.change >= 0 ? "Tiền thừa" : "Còn nợ";
    }
  },
  methods: {
    getProductPrice(item) {
      item.discount = parseFloat(item.discount) || 0;
      const unitPrice = item.base_cost;
    ...
    fetchCustomers(){
      ...
      supplierActions.getSuppliers(payload)
        .then((response) => {
         this.listSupplier = response.data.data.map(customer => ({
            ...customer,
            displayText: `${customer.name} - ${customer.phone}`
          }));
          console.log("Fetched customers:", this.listSupplier);
        })
        .catch((error) => {
