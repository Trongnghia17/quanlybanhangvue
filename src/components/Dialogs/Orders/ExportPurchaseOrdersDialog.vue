...
export default {
  name: 'ExportPurchaseOrdersDialog',
  props: {
    open: {
      type: Boolean,
      default: false
  ...
  data() {
    return {
      ...
      form: {
        from: null,
        to: null,
        type: 2 // Default type for purchase orders
      }
    }
  },
  computed: {
  ...
  methods: {
    ...
    resetForm() {
      this.form = {
        from: null,
        to: null,
        type: 2 // Reset to default type for purchase orders
      };
    },
    dateRangeRule() {
    ...
    async submitExport() {
      ...
      try {
        ...
        const payload = {
          type: 2 // Always include type=2 for purchase orders
        };
        ...
        link.download = `danh_sach_don_nhap_hang_${today}.xlsx`;
        link.click();
